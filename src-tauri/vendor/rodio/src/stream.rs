...
impl OutputStream {
    ...
    /// Pauses the underlying `cpal::Stream` itself, as opposed to `Sink::pause()` (which only
    /// stops feeding a sink real samples, leaving the actual OS-level audio output stream
    /// running and continuing to emit silence). Pausing here is what actually tells the OS
    /// this app has stopped producing sound -- e.g. it's what flips a Windows audio session
    /// from `AudioSessionStateActive` to `AudioSessionStateInactive`, and the equivalent on
    /// macOS/Linux -- letting other apps and OS-level "is this app playing audio" indicators
    /// see the correct state instead of "active" forever until the whole process exits.
    ///
    /// Vendored patch: rodio's own `Sink`/`OutputStream` API has no public way to reach the
    /// underlying `cpal::Stream` (tracked upstream at
    /// <https://github.com/RustAudio/rodio/issues/782>, unresolved as of this vendoring).
    /// This mirrors the plain, minimal wrapper design discussed there, rather than exposing
    /// the raw `cpal::Stream` itself.
    pub fn pause(&self) -> Result<(), cpal::PauseStreamError> {
        self._stream.pause()
    }

    /// Resumes a stream previously paused via [`OutputStream::pause`]. See its docs for why
    /// this exists as a vendored patch rather than a public rodio API.
